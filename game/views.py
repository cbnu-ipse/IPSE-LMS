...
from .models import SlotPlayLog, LobbyChatMessage, AppleGameScore, GameSeason, MemoryMatchScore, NumberSpeedScore, PatternRecallScore, BalanceGameScore
...
def get_balance_ranking(top_n=10, season=None):
    """중심잡기 최고 점수 랭킹. season 지정 시 해당 시즌 기간만 집계."""
    from django.db.models import Max
    qs = BalanceGameScore.objects.values("user")
    if season is not None:
        qs = qs.filter(
            played_at__date__gte=season.start_date,
            played_at__date__lte=season.end_date,
...
@login_required
def balance_game_view(request):
    latest = LobbyChatMessage.objects.select_related("user").order_by("-created_at")[:50]
    chat_messages = list(latest)[::-1]
    return render(request, "game/balance_game.html", {"title": "중심잡기", "chat_messages": chat_messages})


@login_required
@require_POST
def save_balance_score(request):
    try:
        score = int(request.POST.get("score", 0))
        survived_ms = int(request.POST.get("survived_ms", 0))
        stage = int(request.POST.get("stage", 0))
    except (TypeError, ValueError):
        return JsonResponse({"ok": False, "error": "invalid payload"}, status=400)
    if score < 0 or survived_ms < 0 or stage < 0:
        return JsonResponse({"ok": False, "error": "invalid payload"}, status=400)
    BalanceGameScore.objects.create(user=request.user, score=score, survived_ms=survived_ms, stage=stage)
    return JsonResponse({"ok": True})


@login_required
def balance_game_ranking(request):
    """중심잡기 TOP 10 랭킹 JSON (현재 시즌 기준)."""
    season = GameSeason.get_or_create_current()
    rows = get_balance_ranking(10, season=season)
    data = [
        {
            "rank": r["rank"],
            "name": r["user"].display_name,
...
def game_ranking_view(request):
    ...
    if board not in {"slot_game", "apple_game", "memory_match", "number_speed", "pattern_recall", "balance_game"}:
        board = "slot_game"
    ...
    if board == "slot_game":
        ...
        elif board == "balance_game":
            ranking_rows = get_balance_ranking(top_n=None, season=selected_season)
        else:
